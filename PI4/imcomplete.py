...
while running:
    ...
    match currnet_step :
        ...
        case Step.end_time:
            ...
            # current_step = Step.servo_1
            #여기까지 서브모터 2 임
            # 서브모터 1부분은 여쭤봐야할듯(여기서 내가 서브모터 1부분을 자체적으로 해야할지 모르겠음)

        # case Step.fourth_part_grade_classification:
        #     print(Step.fourth_part_grade_classification)
        #     svmotor_step = GuideMotorStep.stop    #기본 stop
        #     if( pass_or_fail ):                 #서버에서 받은 불량기준
        #         motor_step = GuideMotorStep.good
        #     else :
        #         motor_step = GuideMotorStep.fail

        #     motor.doGuideMotor( GuideMotorStep.stop )   #위에 따라서 모터구동 
        #     currnet_step = Step.servo_1

        # case Step.servo_1:
        #     print( Step.servo_1 )
        #     svmotor_step = GuideMotorStep.stop    #기본 stop
        #     if( pass_or_fail ):                 #서버에서 받은 불량기준
        #         svmotor_step = GuideMotorStep.good
        #     else :
        #         svmotor_step = GuideMotorStep.fail
        #     motor.doGuideMotor( GuideMotorStep.stop )    #위에 따라서 모터구동 
        #     currnet_step = Step.go_rail_next_1
